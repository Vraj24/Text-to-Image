...
#             nn.Sigmoid(),  # Added sigmoid to match BCEWithLogitsLoss expectations
#         )

#     def forward(self, img, text_embedding):
#         img_feat = self.img_encoder(img)  # [B, 1024, 8, 8]

#         text = self.text_proj(text_embedding).unsqueeze(2).unsqueeze(3)
#         B, _, H, W = img_feat.shape
#         text = text.expand(-1, -1, H, W)

#         joint = torch.cat((img_feat, text), 1)  # [B, 2048, 8, 8]
#         out = self.out(joint)
#         return out.mean([2, 3]).view(-1)
