...
#     def __init__(self, text_dim=1024, stage1_channels=3):
#         super().__init__()
...
#             nn.Conv2d(stage1_channels, 128, 3, 1, 1),
#             nn.BatchNorm2d(128),
#             nn.ReLU(True)
#         )
...
#         # [B, 1024] → [B, 128, 1, 1]
#         text = self.text_encoder(text_embedding).unsqueeze(2).unsqueeze(3)
#         text = text.repeat(1, 1, stage1_img.size(2), stage1_img.size(3))  # match image size

#         img_feat = self.image_encoder(stage1_img)
#         x = torch.cat((img_feat, text), dim=1)
#         x = self.joint(x)
#         x = self.res_blocks(x)
